...
enum lws_callback_reasons {
	...
	LWS_CALLBACK_FILTER_PROTOCOL_CONNECTION, // 20: 过滤协议连接
	...
	LWS_CALLBACK_ADD_POLL_FD, // 32 将套接字加入事件监听
	LWS_CALLBACK_DEL_POLL_FD, // 33 将套接字移除事件监听
	LWS_CALLBACK_CHANGE_MODE_POLL_FD, // 34 改变监听的事件
	LWS_CALLBACK_LOCK_POLL, // 35 对POLL加锁
	LWS_CALLBACK_UNLOCK_POLL, // 36 对POLL解锁
...
/* 扩展函数 # Qifeng.zou # 2015-12-08 11:05:04 # */
...
LWS_VISIBLE int lws_wsi_get_fd(struct libwebsocket *wsi);
LWS_VISIBLE void *lws_wsi_get_user_space(struct libwebsocket *wsi);
LWS_VISIBLE struct lws_context *lws_wsi_get_ctx(struct libwebsocket *wsi);
